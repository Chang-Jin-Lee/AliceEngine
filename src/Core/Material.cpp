...
namespace Alice
{
    bool MaterialFile::Load(const std::filesystem::path& path, MaterialComponent& outMaterial, ResourceManager* rm)
    {
        ...
        ALICE_LOG_INFO("[MaterialFile] Load: \"%s\" color=(%.3f, %.3f, %.3f) rough=%.3f metal=%.3f | outline=(%.3f, %.3f, %.3f) width=%.3f tex=\"%s\"",
            path.string().c_str(),
    ...
    bool MaterialFile::Save(const std::filesystem::path& path, const MaterialComponent& material)
    {
        ...
        ALICE_LOG_INFO("[MaterialFile] Save: \"%s\" color=(%.3f, %.3f, %.3f) rough=%.3f metal=%.3f | outline=(%.3f, %.3f, %.3f) width=%.3f tex=\"%s\"",
            path.string().c_str(),
