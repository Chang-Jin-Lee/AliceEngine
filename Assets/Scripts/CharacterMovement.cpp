...
//         float mz = 0.0f;
//
//         // 기존 코드: 여기서 rotationY를 직접 대입해서 덮어씌워지는 문제가
//         있었음
...
//
//             // --- 3. 회전 계산 ---
//             // atan2(x, z)는 (0,0)에서 (x,z)를 바라보는 각도를 라디안으로
//             반환합니다. float radian = std::atan2(mx, mz);
//
//             // 라디안 -> 디그리 변환
//             float degree = radian * (180.0f / static_cast<float>(M_PI));
//
...
//             // 작성하신 코드에서 W(전방)일 때 180도를 주고 있으므로, +180도
//             보정 t->SetRotation(0.0f, degree + 180.0f, 0.0f);
//
//             // 걷기 애니메이션 스마트 재생
//             anim.Play(2);
//         }
